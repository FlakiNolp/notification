...
from api.utils import notifications, get_hash_api_token

from api.db_utils import get_user_by_api_token
        ...
        with open(f'{config.logs_path}/{hash_filename}.json', 'r') as f:
            ...
            if message and additional:
                for log in logs['logs']:
                    if log['code'] in code and time_from <= datetime.strptime(log['time'], '%Y-%m-%d %H:%M:%S.%f') <= time_to and log['message'] in message and log['additional'] in additional:
                        result['logs'].append(log)
            elif message:
                for log in logs['logs']:
                    if log['code'] in code and time_from <= datetime.strptime(log['time'], '%Y-%m-%d %H:%M:%S.%f') <= time_to and log['message'] in message:
                        result['logs'].append(log)
            elif additional:
                for log in logs['logs']:
                    if log['code'] in code and time_from <= datetime.strptime(log['time'], '%Y-%m-%d %H:%M:%S.%f') <= time_to and log['additional'] in additional:
                        result['logs'].append(log)
            else:
                for log in logs['logs']:
                    if log['code'] in code and time_from <= datetime.strptime(log['time'], '%Y-%m-%d %H:%M:%S.%f') <= time_to:
                        result['logs'].append(log)
